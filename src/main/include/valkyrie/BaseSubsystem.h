 ...
 * BaseSubsystem is used to organize code and abstract a lot of the base code that is often
 ...
 *
 * The idea is that subsystems on the robot implement BaseSubsystem and logic for that
...
class BaseSubsystem : public frc2::Subsystem, public wpi::Sendable, public wpi::SendableHelper<BaseSubsystem> {
   public:
    ...
    BaseSubsystem(frc::TimedRobot* _robot, const char* name)
        : robot(_robot), subsystemName(name), operatorGamepad(NULL), driverGamepad(NULL) {
