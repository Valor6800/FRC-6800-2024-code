...
PoseTracker::PoseTracker(int _bufferSize) : bufferSize(_bufferSize) {}
    ...
    if (poseBuffer.size() != 0) {
        if ((poseBuffer.back().first - pose).Translation().Norm() > 1_m) {
            ...
            units::meters_per_second_t vel =
                (pose - poseBuffer.back().first).Translation().Norm() / (timestamp - poseBuffer.back().second);
            if (velBuffer.size() != 0) {
                accelBuffer.push_back(
                    {(vel - velBuffer.back().first) / (timestamp - velBuffer.back().second), timestamp});
            }
            velBuffer.push_back({vel, timestamp});
            units::degrees_per_second_t angVel = (pose.Rotation() - poseBuffer.back().first.Rotation()).Degrees() /
                                                 (timestamp - poseBuffer.back().second);
...
void PoseTracker::resetBuffers(uint count) {
    while (poseBuffer.size() > count)
        poseBuffer.pop_front();
    while (velBuffer.size() > count)
        velBuffer.pop_front();
    while (accelBuffer.size() > count)
        accelBuffer.pop_front();
    while (angVelBuffer.size() > count)
        angVelBuffer.pop_front();
}
