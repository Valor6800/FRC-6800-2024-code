...
Shooter::Shooter(frc::TimedRobot* _robot, Drivetrain* _drive, frc::AnalogTrigger* _feederBeamBreak,
                 frc::AnalogTrigger* _feederBeamBreak2, valor::CANdleSensor* _leds)
...

{
    ...
    pathplanner::NamedCommands::registerCommand(
        ...
        std::move(frc2::SequentialCommandGroup(frc2::InstantCommand([this]() {
                                               ...
                                               frc2::WaitCommand(1_s), frc2::InstantCommand([this]() {
                                                   // shooter->state.isShooting = false;
                                                   state.flywheelState = Shooter::FLYWHEEL_STATE::NOT_SHOOTING;
                                               })))
            .ToPtr());

    pathplanner::NamedCommands::registerCommand("Spool",
                                                std::move(frc2::SequentialCommandGroup(frc2::InstantCommand([this]() {
                                                    // shooter->state.isShooting = true;
                                                    state.flywheelState = Shooter::FLYWHEEL_STATE::SHOOTING;
                                                }))).ToPtr());
    pathplanner::NamedCommands::registerCommand("Enable shooter",
                                                std::move(frc2::SequentialCommandGroup(frc2::InstantCommand([this]() {
                                                    // shooter->state.isShooting = true;
                                                    state.flywheelState = Shooter::FLYWHEEL_STATE::SHOOTING;
                                                }))).ToPtr());
    pathplanner::NamedCommands::registerCommand("Disable shooter",
                                                std::move(frc2::SequentialCommandGroup(frc2::InstantCommand([this]() {
                                                    // shooter->state.isShooting = true;
                                                    state.flywheelState = Shooter::FLYWHEEL_STATE::NOT_SHOOTING;
                                                }))).ToPtr());
    pathplanner::NamedCommands::registerCommand("Start pivot tracking", std::move(frc2::InstantCommand([this]() {
                                                                            // shooter->state.isShooting = true;
                                                                            state.pivotState =
                                                                                Shooter::PIVOT_STATE::TRACKING;
                                                                        })).ToPtr());
    pathplanner::NamedCommands::registerCommand("Set pivot subwoofer", std::move(frc2::InstantCommand([this]() {
                                                                           // shooter->state.isShooting = true;
                                                                           state.pivotState =
                                                                               Shooter::PIVOT_STATE::SUBWOOFER;
                                                                       })).ToPtr());
    pathplanner::NamedCommands::registerCommand("Set pivot load", std::move(frc2::InstantCommand([this]() {
                                                                      // shooter->state.isShooting = true;
                                                                      state.pivotState = Shooter::PIVOT_STATE::LOAD;
                                                                  })).ToPtr());
    pathplanner::NamedCommands::registerCommand("Set pivot podium", std::move(frc2::InstantCommand([this]() {
                                                                        // shooter->state.isShooting = true;
                                                                        state.pivotState = Shooter::PIVOT_STATE::PODIUM;
                                                                    })).ToPtr());
    pathplanner::NamedCommands::registerCommand("Set pivot wing", std::move(frc2::InstantCommand([this]() {
                                                                      // shooter->state.isShooting = true;
                                                                      state.pivotState = Shooter::PIVOT_STATE::WING;
                                                                  })).ToPtr());
    pathplanner::NamedCommands::registerCommand("Set pivot orbit", std::move(frc2::InstantCommand([this]() {
                                                                       // shooter->state.isShooting = true;
                                                                       state.pivotState = Shooter::PIVOT_STATE::ORBIT;
                                                                   })).ToPtr());
    pathplanner::NamedCommands::registerCommand(
        "Shoot amp", std::move(frc2::InstantCommand([this]() {
                         state.pivotState =
                             Shooter::PIVOT_STATE::AMP;  // WARNING: Rename to AMP after robot_v2 gets merged in
                     })).ToPtr());
    pathplanner::NamedCommands::registerCommand("Set pivot far low", std::move(frc2::InstantCommand([this]() {
                                                                         // shooter->state.isShooting = true;
                                                                         state.pivotState =
                                                                             Shooter::PIVOT_STATE::AUTO_FAR_LOW;
                                                                     })).ToPtr());
    pathplanner::NamedCommands::registerCommand("Set pivot far high", std::move(frc2::InstantCommand([this]() {
                                                                          // shooter->state.isShooting = true;
                                                                          state.pivotState =
                                                                              Shooter::PIVOT_STATE::AUTO_FAR_HIGH;
                                                                      })).ToPtr());
    pathplanner::NamedCommands::registerCommand("Set pivot near", std::move(frc2::InstantCommand([this]() {
                                                                      // shooter->state.isShooting = true;
                                                                      state.pivotState =
                                                                          Shooter::PIVOT_STATE::AUTO_NEAR;
                                                                  })).ToPtr());
    pathplanner::NamedCommands::registerCommand("Reverse flywheels", std::move(frc2::InstantCommand([this]() {
                                                                         // shooter->state.isShooting = true;
                                                                         state.reverseFlywheels = true;
                                                                     })).ToPtr());
    pathplanner::NamedCommands::registerCommand("Normal flywheels", std::move(frc2::InstantCommand([this]() {
                                                                        // shooter->state.isShooting = true;
                                                                        state.reverseFlywheels = false;
                                                                    })).ToPtr());
    pathplanner::NamedCommands::registerCommand(
        "Set pivot auto subwoofer",
        std::move(frc2::InstantCommand([this]() { state.pivotState = Shooter::PIVOT_STATE::AUTO_SUBWOOFER; })).ToPtr());
}
...
void Shooter::init() {
    ...
    pivotMotors = new valor::PhoenixController(CANIDs::PIVOT, valor::NeutralMode::Brake, false,
                                               PIVOT_GEAR_RATIO / PIVOT_CANCODER_GEAR_RATIO,
                                               PIVOT_CANCODER_GEAR_RATIO / 360.0, pivotPID, 10.0, false, "baseCAN");
...
void Shooter::assessInputs() {
    ...
    // SHOOT LOGIC
    if (driverGamepad->rightTriggerActive() || driverGamepad->leftTriggerActive() ||
        operatorGamepad->GetStartButton() || driverGamepad->GetXButton() ||
...
void Shooter::analyzeDashboard() {
    ...
    if ((xPos > 16.54_m / 2 && frc::DriverStation::GetAlliance() == frc::DriverStation::Alliance::kBlue) ||
        (xPos < 16.54_m / 2 && frc::DriverStation::GetAlliance() == frc::DriverStation::Alliance::kRed)) {
        state.otherSide = true;
    } else {
        state.otherSide = false;
    }
    if ((xPos < 5.85_m && frc::DriverStation::GetAlliance() == frc::DriverStation::Alliance::kBlue) ||
...
void Shooter::calculatePivotAngle() {
    ...
    distance = std::fmin(distance, 9.0);  // Since the parabola has a positive x^2 term, it'll eventually curve up

    double A = -0.433;  // 0;
    double B = 6.42;    // 2.23;
    double C = -33.5;   // -21.3;
    double D = 89.4;    // 78.5;
    double bestPivot = D + (C * distance) + (B * std::pow(distance, 2)) + (A * std::pow(distance, 3));
...
void Shooter::InitSendable(wpi::SendableBuilder& builder) {
    ...
    builder.AddIntegerProperty(
        "flywheel state", [this] { return state.flywheelState; }, nullptr);

    builder.AddIntegerProperty(
        "pivot state", [this] { return state.pivotState; }, nullptr);

    builder.AddDoubleProperty(
        "pivot setpoint", [this] { return state.calculatingPivotingAngle.to<double>(); }, nullptr);
}
