...
void Auto::preloadAuto(std::string autoName) {
    ...
    loadedAutos.push_back({autoName, makeAuto(autoName)});
}
...
std::string makeFriendlyName(std::string filename) {
    ...
    for (uint i = 0; i < filename.length(); i++) {
        ...
        if (filename[i] == '.') {
            ...
            if (*(n_name.end() - 1) != ' ')
                n_name += ' ';
        } else if (i >= 1 && is_alpha(filename[i]) && is_caps(filename[i]) && !is_caps(filename[i - 1]) &&
                   *(n_name.end() - 1) != ' ') {  // check for camel case, add space if present
            n_name += ' ';
            n_name += std::tolower(filename[i]);
        } else if (i == 0) {  // first letter should be capitaized
            n_name += std::toupper(filename[i]);
        } else {
            n_name += std::tolower(filename[i]);
        }
    }
