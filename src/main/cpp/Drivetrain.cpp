...
Drivetrain::Drivetrain(frc::TimedRobot* _robot, valor::CANdleSensor* _leds)
    ...
    std::vector<std::string> pitSequenceNames = {
        ...
        "Set pivot subwoofer",
        "Set pivot podium",
        "Set pivot wing"};  // WARNING: Jank and bad.
                            // Should either be automatically loaded from the auto or the pit sequence should be a list
                            // of commands. Also shouldn't be stored here
    pathplanner::NamedCommands::registerCommand(
        "Wait for A button", std::move(frc2::FunctionalCommand(
                                           ...
                                           [this]() { return operatorGamepad->GetAButtonPressed(); }, {this}))
                                 .ToPtr());
}
...
void Drivetrain::configSwerveModule(int i) {
    ...
    motorLocations[i] = frc::Translation2d{Constants::moduleDiff() * MDX[i], Constants::moduleDiff() * MDY[i]};
    ...
    azimuthControllers.push_back(new SwerveAzimuthMotor(CANIDs::AZIMUTH_CANS[i], valor::NeutralMode::Brake,
                                                        Constants::swerveAzimuthsReversals()[i], AZIMUTH_GEAR_RATIO,
                                                        1.0, azimuthPID, 8.0, false, PIGEON_CAN_BUS));
    ...
    driveControllers.push_back(new SwerveDriveMotor(CANIDs::DRIVE_CANS[i], valor::NeutralMode::Coast,
                                                    Constants::swerveDrivesReversals()[i], 1.0, conversion, drivePID,
                                                    12.0, true, PIGEON_CAN_BUS));
    ...
    swerveModules.push_back(new valor::Swerve<SwerveAzimuthMotor, SwerveDriveMotor>(
        azimuthControllers[i], driveControllers[i], motorLocations[i]));
...
void Drivetrain::init() {
    ...
    pigeon.GetConfigurator().Apply(ctre::phoenix6::configs::Pigeon2Configuration{}.WithMountPose(
    ...
    estimator = new frc::SwerveDrivePoseEstimator<SWERVE_COUNT>(*kinematics, pigeon.GetRotation2d(), getModuleStates(),
                                                                frc::Pose2d{0_m, 0_m, 0_rad});
    calculatedEstimator = new frc::SwerveDrivePoseEstimator<SWERVE_COUNT>(
        *kinematics, pigeon.GetRotation2d(), getModuleStates(), frc::Pose2d{0_m, 0_m, 0_rad});
    ...
    AutoBuilder::configureHolonomic(
        ...
        [this]() {
            ...
            table->PutNumber("timestamp of retrieving speeds", frc::Timer::GetFPGATimestamp().to<double>());
        ...
        HolonomicPathFollowerConfig(  // HolonomicPathFollowerConfig, this should likely live in your Constants class
            ...
            Constants::driveBaseRadius(),  // Drive base radius in meters. Distance from robot center to furthest
                                           // module.
            ReplanningConfig(true, false, .1_m,
                             .25_m)  // Default path replanning config. See the API for the options here
            ),
        []() {
    ...
    pathplanner::NamedCommands::registerCommand("Set Camera Estimator",
                                                std::move(frc2::SequentialCommandGroup(frc2::InstantCommand([this]() {
                                                    state.useCalculatedEstimator = true;
                                                }))).ToPtr());

    pathplanner::NamedCommands::registerCommand("Set Estimator",
                                                std::move(frc2::SequentialCommandGroup(frc2::InstantCommand([this]() {
                                                    state.useCalculatedEstimator = false;
                                                }))).ToPtr());

    pathplanner::NamedCommands::registerCommand("Reset gyro",
                                                std::move(frc2::InstantCommand([this]() { resetGyro(); })).ToPtr());
}

std::vector<valor::Swerve<Drivetrain::SwerveAzimuthMotor, Drivetrain::SwerveDriveMotor>*>
...
double Drivetrain::angleWrap(double degrees) {
    ...
    double start = std::fmod(zeroTo360, 360);  // will work for positive angles
...
void Drivetrain::assessInputs() {
    ...
    state.isHeadingTrack =
        (driverGamepad->leftTriggerActive() && !driverGamepad->GetAButton()) || driverGamepad->GetXButton();
...
void Drivetrain::calculateCarpetPose() {
    calculatedEstimator->UpdateWithTime(frc::Timer::GetFPGATimestamp(), getPigeon(), getModuleStates());
    ...
    calculatedEstimator->AddVisionMeasurement(
        frc::Pose2d{factor * deltaX + previousPose.X(), newPose.Y(), newPose.Rotation()},
        frc::Timer::GetFPGATimestamp(), {0.1, 0.1, 0.1});
...
void Drivetrain::analyzeDashboard() {
    ...
    else if (state.sourceAlign)
        setAlignmentAngle(Alignment::SOURCE);
    else if (state.ampAlign)
        setAlignmentAngle(Alignment::AMP);
    else if (state.trapAlign)
        setAlignmentAngle(Alignment::TRAP);
    else if (state.thetaLock)
        setAlignmentAngle(Alignment::LOCK);

    estimator->UpdateWithTime(frc::Timer::GetFPGATimestamp(), getPigeon(), getModuleStates());
    calculateCarpetPose();
    ...
    for (valor::AprilTagsSensor* aprilLime : aprilTagSensors) {
        aprilLime->applyVisionMeasurement(calculatedEstimator, getRobotSpeeds(),
                                          table->GetBoolean("Accepting Vision Measurements", true), doubtX, doubtY);
    }
    ...
    currentPoseTracker.addReading(frc::Pose2d{units::meter_t{bp[0]}, units::meter_t{bp[1]}, units::radian_t{bp[2]}},
                                  frc::Timer::GetFPGATimestamp());
    targetPoseTracker.addReading(frc::Pose2d{units::meter_t{tp[0]}, units::meter_t{tp[1]}, units::radian_t{tp[2]}},
                                 frc::Timer::GetFPGATimestamp());
...
void Drivetrain::assignOutputs() {
    ...
    state.angleRPS =
        units::angular_velocity::radians_per_second_t{error * kPRot * rotMaxSpeed + (error - state.prevError) * kDRot};
    ...
    if (frc::Timer::GetFPGATimestamp().to<double>() - teleopStart > TIME_TELEOP_VERT &&
        frc::Timer::GetFPGATimestamp().to<double>() - teleopStart < TIME_TELEOP_VERT + 3) {
...
frc::Pose2d Drivetrain::getPoseFromSpeaker() {
    ...
    if (tagSensor->getPoseFromAprilTag().Translation().Norm() < 4.7_m) {
        if (frc::DriverStation::GetAlliance() == frc::DriverStation::kBlue &&
            (tagSensor->getTagID() == 7 || tagSensor->getTagID() == 8)) {
            ...
            return tagSensor->getSensor().ToPose2d();
                   ...
                   (tagSensor->getTagID() == 4 || tagSensor->getTagID() == 3)) {
            leds->setColor(0, valor::CANdleSensor::LIGHT_BLUE);
            table->PutBoolean("good to shoot", true);
            return tagSensor->getSensor().ToPose2d();
        }
...
units::meter_t Drivetrain::getDistanceFromSpeaker() {
    units::meter_t x = frc::DriverStation::GetAlliance() == frc::DriverStation::kBlue
                           ? getPoseFromSpeaker().X() - SPEAKER_BLUE_X
                           : getPoseFromSpeaker().X() - SPEAKER_RED_X;
...
units::meters_per_second_t Drivetrain::getRobotSpeeds() {
    return units::meters_per_second_t{
        sqrtf(powf(getRobotRelativeSpeeds().vx.to<double>(), 2) + powf(getRobotRelativeSpeeds().vy.to<double>(), 2))};
}
...
void Drivetrain::getSpeakerLockAngleRPS() {
    ...
    double speakerX =
        (frc::DriverStation::GetAlliance() == frc::DriverStation::kRed ? SPEAKER_RED_X : SPEAKER_BLUE_X).to<double>();
    ...
    state.targetAngle = units::radian_t(std::atan2((roboYPos.to<double>() - (SPEAKER_Y.to<double>() + speakerYOffset)),
...
void Drivetrain::setAlignmentAngle(Alignment align) {
    ...
    if (align == Alignment::AMP)
        state.targetAngle = BLUE_AMP_ROT_ANGLE;
...
wpi::array<frc::SwerveModulePosition, SWERVE_COUNT> Drivetrain::getModuleStates() {
    wpi::array<frc::SwerveModulePosition, SWERVE_COUNT> modulePositions =
        wpi::array<frc::SwerveModulePosition, SWERVE_COUNT>(wpi::empty_array);
...
frc::Rotation2d Drivetrain::getPigeon() {
    state.accel = {pigeon.GetAccelerationX().GetValue(), pigeon.GetAccelerationY().GetValue(),
...
void Drivetrain::drive(units::velocity::meters_per_second_t vx_mps, units::velocity::meters_per_second_t vy_mps,
                       units::angular_velocity::radians_per_second_t omega_radps, bool isFOC) {
    auto desiredStates = getModuleStates(vx_mps, vy_mps, omega_radps, isFOC);
    setSwerveDesiredState(desiredStates, true);
}

void Drivetrain::driveRobotRelative(frc::ChassisSpeeds speeds) {
    table->PutNumberArray("target chassis speeds", std::vector<double>{speeds.vx.to<double>(), speeds.vy.to<double>(),
                                                                       speeds.omega.to<double>()});
...
wpi::array<frc::SwerveModuleState, SWERVE_COUNT> Drivetrain::getModuleStates(
    units::velocity::meters_per_second_t vx_mps, units::velocity::meters_per_second_t vy_mps,
    units::angular_velocity::radians_per_second_t omega_radps, bool isFOC) {
    frc::ChassisSpeeds chassisSpeeds =
        isFOC ? frc::ChassisSpeeds::FromFieldRelativeSpeeds(vx_mps, vy_mps, omega_radps,
                                                            estimator->GetEstimatedPosition().Rotation())
...
frc::ChassisSpeeds Drivetrain::getRobotRelativeSpeeds() {
    wpi::array<frc::SwerveModuleState, 4> moduleStates = {swerveModules[0]->getState(), swerveModules[1]->getState(),
                                                          swerveModules[2]->getState(), swerveModules[3]->getState()};
    return kinematics->ToChassisSpeeds(moduleStates);
}

void Drivetrain::setSwerveDesiredState(wpi::array<frc::SwerveModuleState, SWERVE_COUNT> desiredStates,
...
frc2::FunctionalCommand* Drivetrain::getResetOdom() {
    return new frc2::FunctionalCommand(
        ...
        [&] {  // continuously running
            ...
            for (valor::AprilTagsSensor* aprilLime : aprilTagSensors) {
                if (aprilLime->hasTarget() &&
                    (aprilLime->getSensor().ToPose2d().X() > 0_m && aprilLime->getSensor().ToPose2d().Y() > 0_m)) {
...
void Drivetrain::setXMode() {
    drive(static_cast<units::velocity::meters_per_second_t>(0), static_cast<units::velocity::meters_per_second_t>(0),
          static_cast<units::angular_velocity::radians_per_second_t>(0), true);
...
frc2::InstantCommand* Drivetrain::getSetXMode() {
    frc2::InstantCommand* cmd_XMode = new frc2::InstantCommand([&] { setXMode(); });
    return cmd_XMode;
}
...
void Drivetrain::InitSendable(wpi::SendableBuilder& builder) {
    ...
    builder.AddDoubleProperty(
        "xSpeed", [this] { return state.xSpeed; }, nullptr);
    builder.AddDoubleProperty(
        "ySpeed", [this] { return state.ySpeed; }, nullptr);
    ...
    builder.AddDoubleArrayProperty(
        ...
        nullptr);
    builder.AddDoubleProperty(
        "rotSpeed", [this] { return state.rot; }, nullptr);

    builder.AddDoubleProperty(
        "xSpeedMPS", [this] { return state.xSpeedMPS.to<double>(); }, nullptr);
    builder.AddDoubleProperty(
        "ySpeedMPS", [this] { return state.ySpeedMPS.to<double>(); }, nullptr);
    builder.AddDoubleProperty(
        "rotSpeedMPS", [this] { return state.rotRPS.to<double>(); }, nullptr);
    builder.AddDoubleProperty(
        "x", [this] { return getPose_m().X().to<double>(); }, nullptr);
    builder.AddDoubleProperty(
        "y", [this] { return getPose_m().Y().to<double>(); }, nullptr);
    builder.AddDoubleProperty(
        "theta", [this] { return getPose_m().Rotation().Degrees().to<double>(); }, nullptr);
    builder.AddBooleanProperty(
        "swerveGood", [this] { return swerveNoError; }, nullptr);
    builder.AddDoubleArrayProperty(
        "pose",
        [this] {
    ...
    builder.AddDoubleArrayProperty(
        ...
        nullptr);
    builder.AddDoubleProperty(
        "pigeonPitch", [this] { return pigeon.GetPitch().GetValueAsDouble(); }, nullptr);
    builder.AddDoubleProperty(
        "pigeoYaw", [this] { return pigeon.GetYaw().GetValueAsDouble(); }, nullptr);
    builder.AddDoubleProperty(
        "pigeonRoll", [this] { return pigeon.GetRoll().GetValueAsDouble(); }, nullptr);
    builder.AddBooleanProperty(
        "Tilted",
        [this] {
    ...
    builder.AddIntegerProperty(
        "stage", [this] { return state.stage; }, nullptr);
    builder.AddIntegerProperty(
        "xVelocity", [this] { return getRobotRelativeSpeeds().vx.to<double>(); }, nullptr);
    builder.AddIntegerProperty(
        "yVelocity", [this] { return getRobotRelativeSpeeds().vy.to<double>(); }, nullptr);
    builder.AddDoubleArrayProperty(
        "swerve states",
        [this] {
    ...
    builder.AddDoubleProperty(
        "targetAngle", [this] { return (units::degree_t(state.targetAngle)).to<double>(); }, nullptr);

    builder.AddDoubleProperty(
        "errorAngle", [this] { return (units::degree_t(getAngleError()).to<double>()); }, nullptr);

    builder.AddDoubleProperty(
        "errorAngleRPS", [this] { return (state.angleRPS).to<double>(); }, nullptr);

    builder.AddDoubleProperty(
        "distanceFromSpeaker", [this] { return (state.distanceFromSpeaker).to<double>(); }, nullptr);
    builder.AddDoubleProperty(
        "velocity current", [this] { return currentPoseTracker.getAverageVelocity().to<double>(); }, nullptr);
    builder.AddDoubleProperty(
        "velocity target", [this] { return targetPoseTracker.getAverageVelocity().to<double>(); }, nullptr);
    builder.AddDoubleProperty(
        "acceleration current", [this] { return currentPoseTracker.getAverageAcceleration().to<double>(); }, nullptr);
    builder.AddDoubleProperty(
        "acceleration target", [this] { return targetPoseTracker.getAverageAcceleration().to<double>(); }, nullptr);
    builder.AddDoubleProperty(
        "angular velocity current", [this] { return currentPoseTracker.getAverageAngularVelocity().to<double>(); },
        nullptr);
    builder.AddDoubleProperty(
        "angular velocity target", [this] { return targetPoseTracker.getAverageAngularVelocity().to<double>(); },
        nullptr);
    builder.AddBooleanProperty(
        "Manually flaged error", [this] { return state.manualFlag; }, nullptr);
    builder.AddDoubleProperty(
        "Bonk!",
        [this] {
            return unfilteredPoseTracker.getAverageAcceleration() >
                   10.0_mps_sq;  // ~60 kg bot -> 600 N, 5 measurements * 20ms = .1s,
                                 // impulse = .1 * 600 = 60 Joules
        },
        nullptr);
    builder.AddBooleanProperty(
        "Pit Mode", [this] { return state.pitMode; }, nullptr);
}
